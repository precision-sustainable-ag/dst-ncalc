...
const Output = () => {
  ...
  if (errorModel || errorCorn) {
    ...
    if (errorModel) {
      errors.push(
        `Couldn't run Model.  Make sure your location is in the continental United States.`
      );
    }
    if (errorCorn) {
  ...
  if (params.get("fy")) {
    const src = `http://aesl.ces.uga.edu/mineralization/client/surface/?fy=${params.get(
      "fy"
    )}&lab=${params.get("lab")}&modeled2=${m2}&modeled4=${m4}&modeled=${mf}`;
  ...
  const options = {
    ...
    tooltip: {
      ...
      formatter: function () {
        ...
        const maxNUptake = Math.max.apply(
          Math,
          NUptake.map((n) => n[1])
        );
    ...
    title: {
      text:
        mockup === 2
          ? outputN === 1 && doCornN
            ? '<div class="caption">Cover crop N released and Corn N uptake over time.</div>'
            : outputN === 1
            ? '<div class="caption">Cover crop N released over time.</div>'
    ...
    yAxis: [
      {
        title: {
          text:
            outputN === 1 && doCornN
              ? `Cover Crop N Released (${unit})<br><div style="color: orange;">Corn N uptake (${unit})</div>`
              : outputN === 1
              ? `Cover Crop N Released (${unit})`
      ...
      */
      {
        ...
        data: doIncorporated
          ? [+incorporatedNPredict, +surfaceNPredict]
          : [+surfaceNPredict],
        color: "#008837",
      },
      {
        ...
        data: doIncorporated
          ? [
              Math.max(0, targetN - incorporatedNPredict),
        ...
        dataLabels: {
          ...
          formatter: function () {
            ...
            return `<div style="color: ${
              this.color
            }; background: white; transform: translateY(${
              doIncorporated ? -38 : -58
            }px);">${this.y} ${unit}${footnote}</div>`;
          },
        },
        animation: false,
      },
  ...
  const residueGraph = {
    ...
    title: {
      text: `<div class="caption">Cover crop residue mass remaining<br>after ${Math.floor(
        model.s.Date.length / (24 * 7)
      )} weeks past termination.</div>`,
      verticalAlign: mockup === 1 ? "bottom" : "top",
    },
    xAxis: {
      categories: doIncorporated
    ...
    series: [
      {
        ...
        data: doIncorporated
          ? [Math.round(incorporatedMin), Math.round(surfaceMin)]
          : [Math.round(surfaceMin)],
        color: "#008837",
      },
    ],
  }; // residueGraph
  ...
  const csv =
    ...
    "\n" +
    dates
      .map((date, i) => date + "," + cols.map((col) => model.s[col][i]))
      .join("\n");
  ...
  const summary = (
    <div
      className="inputs"
      style={{
        borderTop: mockup === 1 ? "1px solid #bbb" : "none",
        paddingTop: mockup === 1 ? "1em" : "none",
      }}
    >
      By &nbsp;
      <select
        ...
        value={nweeks}
      >
        {Array(Math.round(model.s.Date.length / 24 / 7))
          .fill()
          .map((_, i) => (
      ...
      {outputN === 1
        ...
        : " undecomposed residue mass remaining "}
      is:
      <ul>
        <li>
          <strong>
            {Math.round(
              surfaceData[Math.min(nweeks * 7, surfaceData.length - 1)].y
            )}
          </strong>{" "}
          {unit} for surface residues.
        </li>
        {doIncorporated && (
          <li>
            {Math.round(
              incorporatedData[
                Math.min(nweeks * 7, incorporatedData.length - 1)
              ].y
            )}{" "}
            {unit} for incorporated residues.
          </li>
        )}
      </ul>
    </div>
      ...
      <div id="Output" className="mockup">
                        ...
                        <tbody>
                          <tr>
                            <td>
                              ...
                              <br />(
                              <strong>
                                {moment(killDate).format("MMM D, yyyy")}
                              </strong>
                              )<p></p>
                              <u>Dry Biomass</u>
                              <br />(
                              <strong>
                                {(+biomass).toFixed(0)} {unit}
                              </strong>
                              )
                            </td>
                          </tr>
                          <tr>
                            <td>
                              ...
                              <br />(
                              <strong>
                                {((biomass * N) / 100).toFixed(0)} {unit}
                              </strong>
                              )<p></p>
                              <u>Carbohydrates</u>
                              <br />(<strong>{carb.toFixed(0)} %</strong>)
                              <p></p>
                              <u>Holo-cellulose</u>
                              <br />(<strong>{cell.toFixed(0)} %</strong>)
                              <p></p>
                              <u>Lignin</u>
                              <br />(<strong>{lign.toFixed(0)} %</strong>)
                            </td>
                          </tr>
                        </tbody>
                      </table>
        ...
        <div className="output-table-div-mobile">
          ...
          >
            ...
            <div>
              <u>Termination Date</u> : (
              <strong>{moment(killDate).format("MMM D, yyyy")}</strong>)<p></p>
            </div>
            <div>
              <u>Dry Biomass</u> : (
              <strong>
            ...
            <div>
              <u>Carbohydrates</u> : (<strong>{carb.toFixed(0)} %</strong>)
              <p></p>
            </div>
            <div>
              <u>Holo-cellulose</u> : (<strong>{cell.toFixed(0)} %</strong>)
              <p></p>
            </div>
            <div>
              <u>Lignin</u> : (<strong>{lign.toFixed(0)} %</strong>)
            </div>
          </div>
