const transpile = ({FOMkg, FOMpctN, FOMpctCarb, FOMpctCell, FOMpctLign, LitterWaterContent, BD, INppm, hours, stop, temp, RH, rain}) => {
  ...
  const S = {
    ...
    'Critical_FOM': [],
    'ContactFactor': [],
    'measured_FOM_kg_ha_Mean': [],
    'measured_FOM_kg_ha_SD': [],
    'measured_FOMN_kg_ha_Mean': [],
    'measured_FOMN_kg_ha_SD': [],
    'Carb0': [],
    'Cell0': [],
    'Lign0': [],
    'FOM0': [],
  ...
  S.NAllocationFactor = 0.1;
  S.CarbN[t] = S.CarbN[t + 1] = Math.max(0, ((S['InitialFOMN_kg/ha'])-(S['InitialFOMN_kg/ha']*S.FOMpctLign/100))*S.NAllocationFactor*S.FOMpctCarb/100);
  ...
  S.WaterLossFromEvap[t] = S.WaterLossFromEvap[t + 1] = (S.LitterWaterContent[t] <= 0.04) ? 0 :  (S.Dew[t] === 0) && (PREVIOUS('Dew',  0) > 0) ? 0.5 :  (S.Rain[t] === 0) && (PREVIOUS('Rain',  0) > 0) ? 0.7 :  (S.RHChange[t] >= 0) ? 0 :  (S.RHChange[t] > -2.5) ? 0.001 :  (S.k_4[t] > 0.03) ? 0.06 : S.k_4[t];
  ...
  S.CarbK = MAX(0.0011, 0.0053 * S.FOMpctLign - 0.0307);
  S.RMTFAC[t] = S.RMTFAC[t + 1] = S.Temp[t] < 0 ? 0 : (0.357+0.019*S.Temp[t])*EXP(S.LitterMPa[t]*(0.198+0.365*S.Temp[t]**-1));
  S.FON[t] = S.FON[t + 1] = S.CarbN[t]+S.CellN[t]+S.LigninN[t];
  S.CNR[t] = S.CNR[t + 1] = (0.41*S.FOM[t])/(S.FON[t]+S.INkg[t]);
  S.CNRF[t] = S.CNRF[t + 1] = MIN(1, EXP(-0.693 * (S.CNR[t] - 13) / 13));
  S.DeCarb[t] = S.DeCarb[t + 1] = S.CarbK*S.RMTFAC[t]*S.CNRF[t];
  S.GrNom1[t] = S.GrNom1[t + 1] = Math.max(0, S.CarbN[t]*S.DeCarb[t]);
  S.k3 = 0.0035;
  S.CellK = S.k3* EXP(-12*(S.FOMpctLign/100));
  S.DeCell[t] = S.DeCell[t + 1] = S.CellK*S.RMTFAC[t]*S.CNRF[t];
  S.GRNom2[t] = S.GRNom2[t + 1] = Math.max(0, S.CellN[t]*S.DeCell[t]);
  S.LignK = 0.00095;
  S.DeLign[t] = S.DeLign[t + 1] = S.LignK*S.RMTFAC[t]*S.CNRF[t];
  ...
  S.Critical_FOM = 4000;
  S.ContactFactor = MAX(1, S.Critical_FOM/S.FOM[t]);
  S.Carb0 = S.Carb[t];
  S.Cell0 = S.Cell[t];
  S.Lign0 = S.Lign[t];
  S.FOM0 = S.FOM[t];
  S.PredCarbMeasured[t] = S.PredCarbMeasured[t + 1] = Math.max(0, S.Carb0);
  S.PredCellMeasured[t] = S.PredCellMeasured[t + 1] = Math.max(0, S.Cell0);
  S.PredLignMeasured[t] = S.PredLignMeasured[t + 1] = Math.max(0, S.Lign0);
  ...
  for (t = 1; t <= hours; t++) {
    ...
    S.CNRF[t] = S.CNRF[t + 1] = MIN(1, EXP(-0.693 * (S.CNR[t] - 13) / 13));
    S.DeCarb[t] = S.DeCarb[t + 1] = S.CarbK*S.RMTFAC[t]*S.CNRF[t];
    if (!S.DeCarb[t].toFixed) {
      console.log(S.RMTFAC[t], S.CNRF[t], S.DeCarb[t])
      // throw '';
    }
    S.GrNom1[t] = S.GrNom1[t + 1] = Math.max(0, S.CarbN[t]*S.DeCarb[t]);
    S.k3 = 0.0035;
    ...
    S.Noname_2[t] = S.Noname_2[t + 1] = Math.max(0, S.RNAC[t]);
    S.Sat = (1-(S.BD/2.65))/S.BD;
    S.PredCarbMeasured[t] = S.PredCarbMeasured[t + 1] = Math.max(0, S.measured_FOM_kg_ha_Mean[t] * (S.Carb0 / S.FOM0 * (0.928 - 0.803 * (1 - S.measured_FOM_kg_ha_Mean[t] / S.FOM0))));
    S.PredCellMeasured[t] = S.PredCellMeasured[t + 1] = Math.max(0, S.measured_FOM_kg_ha_Mean[t] * (S.Cell0 / S.FOM0 * (1.028 + 0.645 * (1 - S.measured_FOM_kg_ha_Mean[t] / S.FOM0))));
    S.PredLignMeasured[t] = S.PredLignMeasured[t + 1] = Math.max(0, S.measured_FOM_kg_ha_Mean[t] * (S.Lign0 / S.FOM0 * (0.915 + 2.138 * (1 - S.measured_FOM_kg_ha_Mean[t] / S.FOM0))));
  }
